...
public class Logger {
    ...
    @Pointcut("execution( public void cn.bps.service.impl.AccountServiceImpl.saveAccount(..))")
    private void saveAccountPointcut(){}

    @Before("saveAccountPointcut()")
    public void beforeAdvice(){
        System.out.println("Logger类:这是一个前置通知,日志系统正在打开");
    }

    @AfterReturning("saveAccountPointcut()")
    public void returningAdvice(){
        System.out.println("Logger类:这是一个后置通知，日志系统正常关闭");
    }

    @AfterThrowing("saveAccountPointcut()")
    public void throwingAdvice(){
        System.out.println("Logger类:这是一个异常通知，检测到一个异常，打印日志");
    }

    @After("saveAccountPointcut()")
